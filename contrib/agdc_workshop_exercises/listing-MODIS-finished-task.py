...
class ListingStacker(Stacker):
    ...
    def derive_datasets(self, input_dataset_dict, stack_output_info, tile_type_info):
        ...
        if 'MOD09' in input_dataset_dict:
            ds_info = input_dataset_dict['MOD09']
            logger.info("MOD09: for daterange (%s -> %s). Path: %s", ds_info['start_datetime'], ds_info['end_datetime'], ds_info['tile_pathname'])
        else: 
            logger.info("No MOD09 data for daterange (%s -> %s)",  stack_output_info['start_datetime'], stack_output_info['end_datetime'])
