...
class NDVIStacker(Stacker):
    ...
    def derive_datasets(self, input_dataset_dict, stack_output_info, tile_type_info):
        ...
        try:
          ...
          NIR_array = nbar_dataset.GetRasterBand(lookup.band_no['MB2']).ReadAsArray() * lookup.adjustment_multiplier['MB2'] + lookup.adjustment_offset['MB2'] 
          R_array = nbar_dataset.GetRasterBand(lookup.band_no['MB1']).ReadAsArray() * lookup.adjustment_multiplier['MB1'] + lookup.adjustment_offset['MB1']
        except TypeError:   
          return
